...
public class TabIndicator extends ViewGroup {
	...
	private int mTabBackgroundResId = android.R.color.transparent;// tab默认背景资源
	private int mTabTextColorResId = android.R.color.tab_indicator_text; // tab默认文字颜色

	/**
	 * 设置Tab页签的背景资源Id
	 * @param tabBackgroundResId   背景资源选择器
	 */
	...
	/**
	 * 设置Tab页签的文字颜色资源Id
	 * @param tabTextColorResId   color选择器
	 */
	public void setTabTextColorResId(int tabTextColorResId) {
		this.mTabTextColorResId = tabTextColorResId;
	}
	...
	private void addTextTab(final int position, String title) {
		...
		tab.setTextColor(mContext.getResources().getColorStateList(mTabTextColorResId));
		tab.setBackgroundDrawable(mContext.getResources().getDrawable(mTabBackgroundResId));
		...
		if (totalWidth <= widthSize) {// 如果子Tab的总宽度小于PagerTab，则采用平分模式
			...
			for (int i = 0; i < mTabCount; i++) {
				...
				int childHeightMeasureSpec = MeasureSpec
						.makeMeasureSpec(child.getMeasuredHeight(), MeasureSpec.EXACTLY);
				child.measure(childWidthMeasureSpec, childHeightMeasureSpec);
			}
			mMaxScrollX = 0;
			mSplitScrollX = 0;
